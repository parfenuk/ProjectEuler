...
mtype linear_recurrence_value (vector<mtype> coeffs, vector<mtype> initial_values, Lnum N, int Q)
{
    if (N.get_size() == 1 && N[0] < initial_values.size()) {
        mtype f = initial_values[N[0]] % Q;
        if (f < 0) f += Q;
        return f;
    }
