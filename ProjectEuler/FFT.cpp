...
void FFT (vector<ll> &a, bool invert, int pw_size) { // actual size is 2^pw_size
    int n = (int)a.size();
    ...
    for (int len=2; len<=n; len<<=1) {
        ll wlen = invert ? P_inv[pw_size] : P[pw_size];
        for (int i=len; i<pw[pw_size]; i<<=1) {
            wlen = wlen * wlen % Q;
        }
    ...
    if (invert) {
        ll n_inv = Algebra::inverse((ll)n,Q);
        for (int i=0; i<n; i++) {
            a[i] = a[i] * n_inv % Q;
        }
    }
...
vll poly_multiply (const vll &a, const vll &b, int pw_size)
{
    vll fa(a.begin(), a.end());
    vll fb(b.begin(), b.end());
    ...
    for (int i=0; i<n; i++) fa[i] = fa[i]*fb[i] % Q;
    FFT(fa, true, pw_size);
